...
colorfulness_criterion = ColorfulnessLoss().to(device)
...
def train_one_epoch(
    ...
    for L, gt in dataloader:
        ...
        with autocast(device_type=device.type):
            ...
            D_loss = 0.5 * (gan_criterion(d_real, True) + gan_criterion(d_fake, False))
        ...
        with autocast(device_type=device.type):
            ...
            G_adv_loss = gan_criterion(d_fake_for_G, True)
            ...
            G_loss = G_rec_loss + G_percep_loss + 0.001 * G_adv_loss + G_color_loss
